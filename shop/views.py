...
def read_ordered(request, order_num):
    ordered = Order.objects.filter(user_fk = request.user.id, order_num = order_num)
    sum_cost = Order.objects.filter(user_fk = request.user.id, order_num = order_num).aggregate(Sum('cost'))
